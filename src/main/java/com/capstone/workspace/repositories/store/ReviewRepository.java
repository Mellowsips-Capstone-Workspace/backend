...
public interface ReviewRepository extends BaseRepository<Review, UUID> {
    ...
    @Query(value = "SELECT new com.capstone.workspace.models.store.StoreReviewStatisticsModel(AVG(r.point), COUNT(r.id)) FROM Review r WHERE r.order.storeId = :storeId GROUP BY r.order.storeId")
