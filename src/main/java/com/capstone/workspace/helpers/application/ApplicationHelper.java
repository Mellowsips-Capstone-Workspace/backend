...
public class ApplicationHelper {
    ...
    public void validate(Application data) {
        ...
        switch (data.getType()) {
            case CREATE_ORGANIZATION:
                partner = validatePartner(jsonData.get("organization"));
            ...
            case ADD_STORE:
                ...
                String partnerId = userIdentity.getUserType() == UserType.ADMIN ? data.getPartnerId() : userIdentity.getPartnerId();

                partner = mapper.map(partnerService.getOneById(UUID.fromString(partnerId)), EnterprisePartner.class);
                validateStore(jsonData.get("merchant"), partner);
                break;
        }
    }
    ...
    private Partner validatePartner(Object partnerData) {
        ...
        if (partner.getBusinessType() != BusinessType.PERSONAL) {
            int count = 0;

            try {
                partnerService.getByBusinessCode(((HouseholdPartner) partner).getBusinessCode());
            } catch (NotFoundException e) {
                count++;
            }

            try {
                partnerService.getByTaxCode(((HouseholdPartner) partner).getTaxCode());
            } catch (NotFoundException e) {
                count++;
            }

            if (count < 2) {
                throw new ConflictException("Partner with the same business code or tax code already exists");
            }
        }
    ...
    private List<Store> validateStore(Object storeData, Partner partner) {
        ...
        if (storeList.size() > 5) {
            throw new BadRequestException("Exceed max allowed new stores per application");
        }
