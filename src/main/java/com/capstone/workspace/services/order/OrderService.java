...
public class OrderService {
    ...
    public OrderModel create(CreateOrderDto dto) {
        ...
        if (cartDetails.getChangesMessage() != null) {
        ...
        if (cartDetails.getFinalPrice() <= 0) {
            entity.setInitialTransactionMethod(TransactionMethod.NONE);
        } else {
        ...
        if (entity.getInitialTransactionMethod() != TransactionMethod.ZALO_PAY) {
        ...
        if (cartDetails.getFinalPrice() > 0) {
            ...
            orderModel.setLatestTransaction(null);
        }
    ...
    public synchronized Order transition(UUID id, String event, RejectOrderDto dto) {
        ...
        switch (newStatus) {
            ...
            case DECLINED -> {
                ...
                if (entity.getInitialTransactionMethod() != TransactionMethod.NONE && transaction.getStatus() == TransactionStatus.SUCCESS) {
                    throw new BadRequestException("Transaction has been completed. Should mark it as received");
                }
                jobService.handleCustomerFlake(entity.getCreatedBy());
            }
            case REJECTED, CANCELED -> {
                ...
                jobService.revokeVoucher(entity.getId());
                if (currentStatus != OrderStatus.PENDING) {
                    jobService.publishPushNotificationOrderChangesJob(mapper.map(entity, OrderModel.class));
                }

                if (entity.getInitialTransactionMethod() != TransactionMethod.NONE) {
    ...
    public Transaction requestTransaction(UUID id) {
        ...
        if (order.getInitialTransactionMethod() == TransactionMethod.NONE) {
            throw new BadRequestException("This order does not have any transaction");
        }
    ...
    public void expiringOrder(UUID orderId) {
        ...
        if (order.getInitialTransactionMethod() != TransactionMethod.ZALO_PAY) {
        ...
        if (order.getStatus() == OrderStatus.PENDING) {
            ...
            PushNotificationDto dto = PushNotificationDto.builder()
                    ...
                    .shortDescription("Đã hết hạn 15 phút, đơn hàng của bạn sẽ tự động hủy.")
                    .content("Đã hết hạn 15 phút, đơn hàng của bạn sẽ tự động hủy.")
