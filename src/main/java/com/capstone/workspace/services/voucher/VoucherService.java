...
public class VoucherService {
    ...
    private Voucher upsert(UUID id, Object dto) {
        ...
        Voucher entity = mapper.map(dto, Voucher.class);
        entity.setOriginalQuantity(((CreateVoucherDto) dto).getQuantity());
        return entity;
    }
    ...
    public Voucher update(UUID id, UpdateVoucherDto dto) {
        ...
        if (entity.getStartDate().isBefore(now)) {
            ...
            BeanUtils.copyProperties(dto, entity, "discountType", "startDate", "minOrderAmount", "maxDiscountAmount", "isHidden", "value", "originalQuantity");
        } else {
            if (dto.getDiscountType() == VoucherDiscountType.CASH) {
    ...
    public PaginationResponseModel<VoucherModel> search(SearchVoucherDto dto) {
        String[] searchableFields = new String[]{"code"};
    ...
    public Voucher useVoucher(UUID id) {
        ...
        while (true) {
            try {
                count++;
                Voucher entity = repository.getAvailableVoucherById(id, userIdentity.getUsername());
                if (entity == null) {
                    throw new NotFoundException("Voucher " + entity.getCode() + " is not available anymore");
                }
